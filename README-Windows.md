...
3. 現在輸入以下指令：
   - **一般情況（自動偵測裝置）：**
     ...
     ```
     - 只有一台裝置時：**自動連線**，不需手動選擇。
     - 偵測到多台裝置時：視窗會列出選單（`[1]`、`[2]`...），輸入編號按 `Enter` 即可選擇。
     - 想要直接指定裝置 UDID：`python gps_spoof.py --serial <UDID>`
     - 查看目前偵測到的所有裝置：`python gps_spoof.py --list-devices`

   - **或是手動指定通道（若有使用視窗 1 的 RSD 資訊）：**
     ```
